import streamlit as st from openai import OpenAI import json import datetime
...
with tab2: if st.button("Genereer Toets"): with st.spinner("Vragen maken..."): p = f"Maak 3 unieke MCQ vragen voor {subject}. Zorg dat alle 4 de opties per vraag verschillend zijn. Geef ALLEEN JSON terug: {{"q": [{{'question': '...', 'options': ['optie1', 'optie2', 'optie3', 'optie4'], 'answer': 'optie1', 'explanation': '...'}}]}}" res = client.chat.completions.create(model="gpt-4o", messages=[{"role": "user", "content": p}], response_format={"type": "json_object"}) st.session_state.quiz = json.loads(res.choices[0].message.content)["q"] st.rerun()
